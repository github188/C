...
enum _OperationCode {
	 ...
	 * sequence number ( CmdSN) for the session and the expected
	 ...
	 * session identifies an I_T nexus). For task sets, the CmdSN of the Task Management
	 ...
	 * arbitrarily selected connection-ID (CID) that is unique
	 ...
	 * Requests/Responses carry text information using a simple "key=value"
	 * syntax. Text Request/Responses may form extended sequences using the same
	 * Initiator Task Tag. The initiator uses the F (Final) flag
